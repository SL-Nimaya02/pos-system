/**
 * schema.planetscale.ts
 *
 * PlanetScale-safe version of schema.ts.
 * ALL foreign key constraints (.references()) have been removed because
 * PlanetScale does not support FK constraints (it uses Vitess which shards
 * across multiple MySQL nodes, making cross-shard FK enforcement impossible).
 *
 * Relationship integrity is enforced at the application layer instead.
 * Drizzle ORM relations (declared below) are unaffected — they are
 * TypeScript-only metadata used for the relational query API, not DB-level FKs.
 *
 * To use this schema:
 *   1. Set DB_MODE=planetscale in your environment.
 *   2. Point drizzle.config.ts to this file when running migrations against PS:
 *        schema: "./src/server/db/schema.planetscale.ts"
 *   3. The runtime db/index.ts already switches to the HTTP driver automatically.
 *
 * Removed FK summary (original relationship → soft-ref column kept):
 *   cashMovements.sessionId          → cashRegisterSessions.id  (CASCADE)
 *   products.categoryId              → categories.id
 *   productVariants.productId        → products.id              (CASCADE)
 *   loyaltyTransactions.loyaltyAccountId → loyaltyAccounts.id
 *   customerCreditTransactions.customerId → loyaltyAccounts.id
 *   orders.sessionId                 → cashRegisterSessions.id
 *   orderItems.orderId               → orders.id                (CASCADE)
 *   orderItems.productId             → products.id
 *   purchaseOrders.supplierId        → suppliers.id
 *   purchaseOrderItems.purchaseOrderId → purchaseOrders.id      (CASCADE)
 *   purchaseOrderItems.productId     → products.id
 *   goodsReceipts.supplierId         → suppliers.id
 *   goodsReceiptItems.grnId          → goodsReceipts.id         (CASCADE)
 *   goodsReceiptItems.productId      → products.id
 *   supplierTransactions.supplierId  → suppliers.id             (CASCADE)
 *   stockBatches.productId           → products.id              (CASCADE)
 */
...
export const cashMovements = mysqlTable("cash_movements", {
  id: varchar("id", { length: 36 }).primaryKey().$defaultFn(() => crypto.randomUUID()),
  // FK removed: originally referenced cashRegisterSessions.id (CASCADE)
...
export const products = mysqlTable("products", {
  ...
  // FK removed: originally referenced categories.id
...
export const productVariants = mysqlTable("product_variants", {
  id: varchar("id", { length: 36 }).primaryKey().$defaultFn(() => crypto.randomUUID()),
  // FK removed: originally referenced products.id (CASCADE)
...
export const loyaltyTransactions = mysqlTable("loyalty_transactions", {
  id: varchar("id", { length: 36 }).primaryKey().$defaultFn(() => crypto.randomUUID()),
  // FK removed: originally referenced loyaltyAccounts.id
  loyaltyAccountId: varchar("loyalty_account_id", { length: 36 }).notNull(),
...
export const orders = mysqlTable("orders", {
  ...
  creditAccountId: varchar("credit_account_id", { length: 36 }),
  ...
  // FK removed: originally referenced cashRegisterSessions.id
...
export const orderItems = mysqlTable("order_items", {
  id: varchar("id", { length: 36 }).primaryKey().$defaultFn(() => crypto.randomUUID()),
  // FK removed: originally referenced orders.id (CASCADE)
  orderId: varchar("order_id", { length: 36 }).notNull(),
  // FK removed: originally referenced products.id
  productId: varchar("product_id", { length: 36 }).notNull(),
...
export const purchaseOrders = mysqlTable("purchase_orders", {
  ...
  // FK removed: originally referenced suppliers.id
...
export const purchaseOrderItems = mysqlTable("purchase_order_items", {
  id: varchar("id", { length: 36 }).primaryKey().$defaultFn(() => crypto.randomUUID()),
  // FK removed: originally referenced purchaseOrders.id (CASCADE)
...
export const goodsReceiptItems = mysqlTable("goods_receipt_items", {
  id: varchar("id", { length: 36 }).primaryKey().$defaultFn(() => crypto.randomUUID()),
  // FK removed: originally referenced goodsReceipts.id (CASCADE)
  grnId: varchar("grn_id", { length: 36 }).notNull(),
  // FK removed: originally referenced products.id
  productId: varchar("product_id", { length: 36 }).notNull(),
  productName: varchar("product_name", { length: 200 }).notNull(),
...
export const supplierTransactions = mysqlTable("supplier_transactions", {
  id: varchar("id", { length: 36 }).primaryKey().$defaultFn(() => crypto.randomUUID()),
  // FK removed: originally referenced suppliers.id (CASCADE)
...
// ─── Audit Logs ───────────────────────────────────────────────────────────────
// userId is a soft ref (no FK) in both versions — unchanged.
...
// ─── Relations (unchanged — these are TypeScript-only, not DB-level FKs) ──────
